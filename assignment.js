...
function tinyFriend(names){
    var checker= names[0].length;
    for(var i=0; i<names.length; i++){
        var words= names[i].length;
        if(words<=checker){
            checker= words;
            var smallest= names[i];
        }
    }
    return smallest;
}
var jigaTolarAdda= tinyFriend(['rakiib','sakkkib','jaoleel','ontor','khairu', 'miru']);
console.log(jigaTolarAdda);